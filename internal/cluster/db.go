...
const (
	dbClusterStateKey = "state"
	dbEnclaveKey      = ".enclave"
	dbClusterRootKey  = "root"
)

func writeState(tx *bolt.Tx, rootKey crypto.SecretKey, state State) error {
	b, err := tx.CreateBucketIfNotExists([]byte(dbClusterBucket))
	if err != nil {
		return err
	}
	plaintext, err := msgp.Marshal[msgp.ClusterState](&state)
	if err != nil {
		return err
	}

	associatedData := []byte(dbClusterBucket + "/" + dbClusterStateKey)
	ciphertext, err := rootKey.Encrypt(plaintext, associatedData)
	if err != nil {
		return err
	}
	return b.Put([]byte(dbClusterStateKey), ciphertext)
}
...
func createEnclave(tx *bolt.Tx, rootKey crypto.SecretKey, name string, enclave *Enclave) error {
	...
	plaintext, err := msgp.Marshal[msgp.Enclave](enclave)
	if err != nil {
		return err
	}
...
func readEnclave(tx *bolt.Tx, rootKey crypto.SecretKey, enclave string) (*Enclave, error) {
	...
	var enc Enclave
	if err = msgp.Unmarshal[msgp.Enclave](plaintext, &enc); err != nil {
		return nil, err
	}
	return &enc, nil
}
...
func createSecretKeyRing(tx *bolt.Tx, enclaveKey crypto.SecretKey, enclave, name string, ring *crypto.SecretKeyRing) error {
	...
	plaintext, err := msgp.Marshal[msgp.SecretKeyRing](ring)
	if err != nil {
		return err
	}
...
func readSecretKeyRing(tx *bolt.Tx, enclaveKey crypto.SecretKey, enclave, name string) (*crypto.SecretKeyRing, error) {
	...
	if err = msgp.Unmarshal[msgp.SecretKeyRing](plaintext, &ring); err != nil {
		return nil, err
	}
	return &ring, nil
}
...
func createPolicy(tx *bolt.Tx, enclaveKey crypto.SecretKey, enclave, name string, policy *auth.Policy) error {
	...
	plaintext, err := msgp.Marshal[msgp.Policy](policy)
	if err != nil {
		return err
	}
...
func readPolicy(tx *bolt.Tx, enclaveKey crypto.SecretKey, enclave, name string) (*auth.Policy, error) {
	...
	if err = msgp.Unmarshal[msgp.Policy](plaintext, &policy); err != nil {
		return nil, err
	}
	return &policy, nil
}
...
func createIdentity(tx *bolt.Tx, enclaveKey crypto.SecretKey, enclave string, identity kes.Identity, info *auth.Identity) error {
	...
	if parent != nil {
		...
		if err = msgp.Unmarshal[msgp.Identity](plaintext, &parentInfo); err != nil {
			return err
		}
		...
		plaintext, err = msgp.Marshal[msgp.Identity](&parentInfo)
		if err != nil {
			return err
		}
		ciphertext, err := enclaveKey.Encrypt(plaintext, associatedData)
	...
	plaintext, err := msgp.Marshal[msgp.Identity](info)
	if err != nil {
		return err
	}
...
func readIdentity(tx *bolt.Tx, enclaveKey crypto.SecretKey, enclave, name string) (*auth.Identity, error) {
	...
	if err = msgp.Unmarshal[msgp.Identity](plaintext, &id); err != nil {
		return nil, err
	}
...
func createSecret(tx *bolt.Tx, enclaveKey crypto.SecretKey, enclave, name string, secret *crypto.Secret) error {
	...
	plaintext, err := msgp.Marshal[msgp.Secret](secret)
	if err != nil {
		return err
	}
...
func readSecret(tx *bolt.Tx, enclaveKey crypto.SecretKey, enclave, name string) (*crypto.Secret, error) {
	...
	if err = msgp.Unmarshal[msgp.Secret](plaintext, &secret); err != nil {
		return nil, err
	}
	return &secret, nil
}
