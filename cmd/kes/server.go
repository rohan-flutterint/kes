...
Options:
    ...
    --host <host:port>       The public IP or FQDN and port of this KES server.
                             ...
                             'localhost' ad the port specified by '--addr'.
...
The $KES_SEAL_KEY is responsible for sealing and unsealing your KES cluster by
decrypting the root encryption key. The same $KES_SEAL_KEY must be present on
all KES servers within a KES cluster. You must generate your own $KES_SEAL_KEY
and keep it secure:
...
Examples:
  1. Start a single node KES cluster accessible at 'localhost:7373'
     $ export KES_SEAL_KEY=kes:v1:aes256:H2BFEgK48Mr4KfuBkxUFJJJNn8f+J0ugpn43ZYJfw30= # Use your own
     $ kes server /tmp/kes0

  2. Start a single node KES cluster accessible at 'kes-0.local:7373'
     $ export KES_SEAL_KEY=kes:v1:chacha20:OYmWIhY6iZAMOqNt610dqit5j/NuZNZc71+XreVdwug= # Use your own
     $ kes server --host kes-0.local:7373 ~/kes0

  3. Start a single node KES cluster accessible at '10.1.2.3:443'
     $ export KES_SEAL_KEY=kes:v1:aes256:OBwNvb9qloQi235KindURBuSqArJzWN6JaYKUZHIyPY= # Use your own
...
func serverCmd(args []string) {
	...
	nodeAddr, err := cluster.ParseNodeAddr(hostFlag)
	if err != nil {
...
func startServer(path, addr string, nodeAddr cluster.NodeAddr, configFile string) {
	ctx, cancelCtx := signal.NotifyContext(context.Background(), syscall.SIGINT, syscall.SIGTERM)
	defer cancelCtx()

	rawSealKey, ok := os.LookupEnv("KES_SEAL_KEY")
	if !ok {
		cli.Fatal("env variable 'KES_SEAL_KEY' is not present")
	}
	sealKey, err := crypto.ParseSealKey(rawSealKey)
	if err != nil {
		cli.Fatalf("invalid env variable 'KES_SEAL_KEY': %v", err)
	}
	...
	if err = node.Start(ctx, path, &cluster.NodeConfig{
		Addr:    nodeAddr,
		SealKey: sealKey,
		Admin:   config.Admin,
