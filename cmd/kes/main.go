...
Options:
    -v, --version            Print version information
        ...
        --gen-seal-key       Generate a new seal key. A seal key
                             decrypts the root key of a cluster
...
func main() {
	...
	cmd.BoolVar(&genSealKey, "gen-seal-key", false, "")
	...
	if genSealKey {
		sealKey, err := crypto.GenerateSealKey(nil)
		if err != nil {
			cli.Fatalf("failed to generate seal key: %v", err)
		}

		bold := tui.NewStyle().Bold(true)
		var buf cli.Buffer
		buf.Sprintln("Your seal key:").Sprintln()
		buf.Styleln(bold, "  ", sealKey.String()).Sprintln()
		buf.Sprintln("This is the only time it is shown. Keep it secret and secure!").Sprintln()
		buf.Sprintln("The seal key protects your KES cluster as unseal mechanism by")
		...
		buf.Sprintln("manager. Without your seal key you cannot decrypt any data on")
		buf.Sprintln("witin KES cluster.")
		cli.Print(buf.String())
		return
	}
