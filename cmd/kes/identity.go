...
func createIdentityCmd(args []string) {
	...
	if err := enclave.CreateIdentity(ctx, identity, &kes.IdentityOptions{
		Policy: policyName,
		Admin:  admin,
		TTL:    ttl,
	}); err != nil {
...
func describeIdentityCmd(args []string) {
	...
	if cmd.NArg() == 0 {
		...
		if !info.CreatedAt.IsZero() {
			...
			buf.Sprintf("Date     : %04d-02%d-02%d %02d:%02d:%02d %s", year, month, day, hour, min, sec, zone).Sprintln()
		}
		if info.TTL > 0 {
		...
		if !info.ExpiresAt.IsZero() {
			...
			buf.Sprintf("Expires  : %04d-02%d-02%d %02d:%02d:%02d %s", year, month, day, hour, min, sec, zone).Sprintln()
		}
